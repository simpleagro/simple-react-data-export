...
import { Button, Input, Form, Affix, Checkbox, Card, Icon, Divider, Popconfirm, Tooltip } from "antd";
    ...
    if (id) {
      ...
      if (formData)
        this.setState(prev => ({
          ...
          group_caracteristicas: group_data.caracteristicas? group_data.caracteristicas.filter(item => !item.deleted) : [],
  ...
  removerVariacao = (variacao) => {
    if (variacao) {
      this.setState(prev => {
        if(prev.formData.variacoes)
          return ({
            formData: {...prev.formData, variacoes: prev.formData.variacoes.filter(item => item !== variacao) }
          })
      });
    }
  };
    ...
    return (
      <div>
        ...
        <Form onChange={this.handleFormState}>
          ...
          {this.state.group_fields && this.gerarFormulario(this.state.group_fields, getFieldDecorator, formItemLayout)}
