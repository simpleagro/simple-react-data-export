...
class Modules extends Component {
  ...
  async initializeList(aqp) {
    ...
    const data = await ModuleService.list(aqp);
  ...
  changeStatus = async (id, newStatus) => {
    try {
      ...
      flashWithSuccess(
        "",
        `O módulo, ${recordName}, foi ${
          newStatus ? "ativado" : "bloqueado"
        } com sucesso!`
      );
    } catch (err) {
      if (err && err.response && err.response.data) parseErrors(err);
      console.log("Erro interno ao mudar status do módulo", err);
    }
  };

  removeRecord = async ({ _id, nome }) => {
    try {
      await ModuleService.remove(_id);
      ...
      flashWithSuccess("", `O módulo, ${nome}, foi removido com sucesso!`);
    } catch (err) {
      if (err && err.response && err.response.data) parseErrors(err);
      console.log("Erro interno ao remover um módulo", err);
    }
  };
  ...
  tableConfig = () => [
    ...
    },
    {
      ...
      render: (text, record) => {
        ...
        return (
          <Popconfirm
            title={`Tem certeza em ${statusTxt} o módulo?`}
            onConfirm={e => this.changeStatus(record._id, !record.status)}
            okText="Sim"
            cancelText="Não"
          >
            <Tooltip title={`${statusTxt.toUpperCase()} o módulo`}>
              <Button size="small">
      ...
      render: (text, record) => {
        return (
          <span>
            ...
            <Popconfirm
              title={`Tem certeza em excluir o módulo?`}
