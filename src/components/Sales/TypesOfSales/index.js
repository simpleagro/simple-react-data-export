...
class SalesType extends Component {
  ...
  async initializeList(aqp) {
    ...
    const data = await SalesTypeService.list(aqp);
  ...
  changeStatus = async (id, newStatus) => {
    try {
      await SalesTypeService.changeStatus(id, newStatus);
      ...
      flashWithSuccess(
        "",
        `O tipo de venda, ${recordName}, foi ${
          newStatus ? "ativado" : "bloqueado"
        } com sucesso!`
      );
    } catch (err) {
      if (err && err.response && err.response.data) parseErrors(err);
      console.log("Erro interno ao mudar status do tipo de venda", err);
    }
  };

  removeRecord = async ({ _id, descricao }) => {
    try {
      await SalesTypeService.remove(_id);
      ...
      flashWithSuccess("", `O tipo de venda, ${descricao}, foi removido com sucesso!`);
    } catch (err) {
      if (err && err.response && err.response.data) parseErrors(err);
      console.log("Erro interno ao remover um tipo de venda", err);
    }
  };
