...
class WarrantyType extends Component {
  constructor(props) {
    super(props);
    this.state = {
      list: [],
  ...
  async initializeList(aqp) {
    ...
    const data = await WarrantyTypeService.list(aqp);
  ...
  changeStatus = async (id, newStatus) => {
    try {
      await WarrantyTypeService.changeStatus(id, newStatus);
      ...
      flashWithSuccess(
        "",
        `O tipo de garantia, ${recordName}, foi ${
          newStatus ? "ativado" : "bloqueado"
        } com sucesso!`
      );
    } catch (err) {
      if (err && err.response && err.response.data) parseErrors(err);
      console.log("Erro interno ao mudar status do tipo de garantia", err);
    }
  };
      ...
      render: (text, record) => {
        return (
          <span>
            ...
            <Popconfirm
              title={`Tem certeza em excluir o tipo de garantia?`}
