...
import * as TypeOfPaymentService from "../../../services/type-of-payment";
...
class TypeOfPayment extends Component {
  ...
  async initializeList(aqp) {
    ...
    const data = await TypeOfPaymentService.list(aqp);
  ...
  changeStatus = async (id, newStatus) => {
    try {
      await TypeOfPaymentService.changeStatus(id, newStatus);
      ...
      flashWithSuccess(
        "",
        `O tipo de pagamento, ${recordName}, foi ${
          newStatus ? "ativado" : "bloqueado"
        } com sucesso!`
      );
    } catch (err) {
      if (err && err.response && err.response.data) parseErrors(err);
      console.log("Erro interno ao mudar status do tipo de pagamento", err);
    }
  };
