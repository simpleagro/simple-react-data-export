...
import * as TargetsSellersService from "../../../services/targets.sellers";
import * as TargetsService from "../../../services/targets";
import * as TargetsGroupService from "../../../services/targets.group";
...
class Sellers extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      meta_id: this.props.match.params.target_id || 0,
      meta_data: {},
      visible: false,
  ...
  async initializeList(aqp) {
    ...
    const data = await TargetsSellersService.list(this.state.meta_id)(aqp);
    const targetData = await TargetsService.get(this.state.meta_id);
  ...
  changeStatus = async (id, newStatus) => {
    try {
      await TargetsSellersService.changeStatus(this.state.meta_id)(
        id,
        newStatus
      );
  ...
  changeStatusGrupo = async (id, newStatus, vendedor_id) => {
    try {
      await TargetsGroupService.changeStatus(this.state.meta_id)(vendedor_id)(id, newStatus);
  ...
  removeRecord = async ({ id, nome }) => {
    try {
      await TargetsSellersService.remove(this.state.meta_id)(id);
  ...
  removeGroup = async ({ id, nome }, vendedor_id) => {
    try {
      await TargetsGroupService.remove(this.state.meta_id)(vendedor_id)(id);
      let _list = this.state.list.map(vendedor => {
        if(vendedor.id == vendedor_id){
            ...
            <span>
              ...
              <Tooltip title="Veja os produtos do grupo de produto">
                <Button
                  size="small"
                  onClick={() =>
                    this.props.history.push(`/metas/${this.state.meta_id}/vendedores/${tabela.id}/grupos-produto/${record.id}/produtos`) 
                  }
                >
  ...
  handleOk = async (item) => {
    ...
    if (!this.state.editMode) {
      ...
      try {
        const created = await TargetsSellersService.create(this.state.meta_id)(obj);
      ...
      try {
        const updated = await TargetsSellersService.update(this.state.meta_id)(obj);
        const data = await TargetsSellersService.list(this.state.meta_id)();
  ...
  handleOkGroup = async (item) => {
    ...
    if (!this.state.editMode) {
      /* if (Object.keys(this.state.formData).length === 0)
        flashWithSuccess("Sem alterações para salvar", " "); */
      try {
        const created = await TargetsGroupService.create(this.state.meta_id)(this.state.vendedor_id)(item.grupo_produto);
        ///target/:target_id/saleman/:saleman_id/productgroup
        this.setState(prev => {
  ...
  handleOkVendedor = async (item) => {
    ...
    if (this.state.editMode) {
      try {
        ...
        flashWithSuccess(
          "",
          `O cabeçalho da meta foi atualizado com sucesso!`
        );
      } catch (err) {
        if (err && err.response && err.response.data) parseErrors(err);
        console.log("Erro interno ao atualizar um cabeçalho de meta ", err);
      } finally {
        this.setState({ savingForm: false });
      }
    }
  ...
  render() {
    return (
      <div>
        {this.props.match.params.target_id ? (
          <div>
            <SimpleBreadCrumb to={"/metas"} history={this.props.history} />
            <Row gutter={24}>
              <Col span={5}>
                <Card
                  bordered
                  ...
                  <p>{`Meta: ${this.state.meta_data.nome || ''}`}</p>
                  <Button
                    style={{ width: "100%" }}
                    onClick={() => { this.showModalVendedor(this.state.meta_data)}}>
