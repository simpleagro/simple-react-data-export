...
import * as ProductsService from "../../../services/targets.products";
import * as TargetSellersService from "../../../services/targets.sellers";
...
class ProductsTarget extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      target_id: this.props.match.params.target_id || 0,
  ...
  async initializeList(aqp) {
    ...
    const data = await ProductsService.list(this.state.target_id)(this.state.saleman_id)(this.state.productgroup_id)(aqp);
    const vendedorData = await TargetSellersService.get(this.state.target_id)(this.state.saleman_id);
  ...
  changeStatus = async (id, newStatus) => {
    try {
      await ProductsService.changeStatus(this.state.target_id)(this.state.saleman_id)(this.state.productgroup_id)(
        id,
        newStatus
      );
  ...
  removeRecord = async ({ id, nome }) => {
    const _id = id;
    try {
      await ProductsService.remove(this.state.target_id)(this.state.saleman_id)(this.state.productgroup_id)(_id);
  ...
  handleOk = async (item) => {
    ...
    if (!this.state.editMode) {
      ...
      try {
        const created = await ProductsService.create(this.state.target_id)(this.state.saleman_id)(this.state.productgroup_id)(obj);
      ...
      try {
        const updated = await ProductsService.update(this.state.target_id)(this.state.saleman_id)(this.state.productgroup_id)(obj);
        const data = await ProductsService.list(this.state.target_id)(this.state.saleman_id)(this.state.productgroup_id)();
    ...
    return (
      <div>
        {this.props.match.params.target_id ? (
          <div>
            <SimpleBreadCrumb to={`/metas/${this.state.target_id}/vendedores`} history={this.props.history} />
            <Row gutter={24}>
              <Col span={5}>
                <Card
                  bordered
...
export default withRouter(ProductsTarget);
